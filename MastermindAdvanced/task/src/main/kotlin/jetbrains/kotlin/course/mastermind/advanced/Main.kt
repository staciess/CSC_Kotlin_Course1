...
fun isCorrectInput(userInput: String, wordLength: Int, alphabet: String): Boolean {
    ...
    if (!userInput.all { it in alphabet }) {
        println("All symbols in your guess should be in the $alphabet alphabet! Try again!")
        return false
    }
    return true
}
...
fun safeUserInput(wordLength: Int, alphabet: String): String {
    while (true) {
        ...
        if (isCorrectInput(input, wordLength, alphabet)) {
            return input
        }
    }
}
